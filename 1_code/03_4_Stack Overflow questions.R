...
posts <- questions %>%
  bind_rows(answers) # first observations' question id have NA because those are questions originally, and only the answers table has that column.
...
# Creating a date variable using lubridate package in Tidyverse
library(lubridate)
#year() function takes a date and turns it into a relevant year.
...
post_with_tags <- bind_rows(questions_with_tags %>% mutate(type="question"),
                            answers_with_tags %>% mutate(type = "answer"))
# Add a year column, then aggregate by type, year, and tag_name
by_type_year_tag <- post_with_tags %>% 
  mutate(year = year(creation_date)) %>%
  count(type, year, tag_name)
# Visualize questions and answers in tags
